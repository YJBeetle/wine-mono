...
wine reg query 'HKCR\MonoTests.RegistrationServices.Probe' > "$test_root/progid.log"
wine reg query 'HKCR\CLSID\{5E4466A3-2BA4-414E-B70B-317D91BE57CC}\InprocServer32' > "$test_root/clsid.log"
wine reg query 'HKCR\CLSID\{5E4466A3-2BA4-414E-B70B-317D91BE57CC}\MonoRegistrationProbe' > "$test_root/callback.log"
...
if wine reg query 'HKCR\MonoTests.RegistrationServices.Probe' > "$test_root/progid-after-unregister.log" 2>&1; then
    echo 'ProgID remains after unregistration' >&2
    exit 1
fi
if wine reg query 'HKCR\CLSID\{5E4466A3-2BA4-414E-B70B-317D91BE57CC}' > "$test_root/clsid-after-unregister.log" 2>&1; then
